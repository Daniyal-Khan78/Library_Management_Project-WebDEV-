...
class BookViewSet(viewsets.ModelViewSet):
    ...
    def return_book(self, request, pk=None):
        ...
        if record:
            # Block return if there is an outstanding unpaid fine
            current_fine = record.calculate_fine()
            if current_fine > 0 and not record.fine_paid:
                return Response(
                    {'error': f'You must pay the outstanding fine of ${current_fine:.2f} before returning this book.'},
